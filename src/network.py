...
def RoadMap_From_Roads(
	roadmap_path='Data/RoadMap/Cali_Roads.pkl',
	...
	with open(roadmap_path,'rb') as file:
...
def Vertices_From_Chargers(chargers):

	x,y=np.array([[c['Longitude'],c['Latitude']] for c in chargers.values()]).T
	charger_ids=np.array([key for key in chargers.keys()])
	...
	for idx in range(n):
		...
		edges=[]
		# print(chargers[charger_id])

		for key,val in chargers[charger_id]['Adjacency'].items():

			edge={
			'vertices':[charger_id,key],
			**val,
			}
...
def Add_Edge_Proximity(graph,gamma=1,min_value=0,max_value=np.inf,
	field='length',name='proximity'):
	'''
	For each edge in a graph, add proximity.
	Proximity is defined as

	proximity=exp(-x/gamma)

	where x is a distance and gamma is the reference distance.
	Where x is equal to gamma proximity will be exp(-1)~=0.368
	'''

	for node_0,edges in graph._adj.items():

		for node_1,edge in edges.items():

			if edge[field]<=min_value:
				proximity=1
			else:
				proximity=np.exp(-edge[field]/(gamma-min_value))*(edge[field]<=max_value)
			edge[name]=proximity

	return graph

def Extract_Edge_Information(graph,field):

	subset=list(graph.nodes)

	return [np.array([edge[field] for edge in graph._adj[node].values()]) for node in subset]


def Add_Edge_Proximity_Normalized(
	graph,gamma=1,max_value=np.inf,field='length',name='proximity'):
	'''
	For each edge in a graph, add proximity.
	Proximity is defined as
	...
	for node_0,edges in graph._adj.items():
		...
		for node_1,edge in edges.items():
			...
			proximity=np.exp(-edge[field]/gamma)*(edge[field]<=max_value)
			edge[name]=proximity

	return graph

def Add_Edge_Proximity_1(graph,gamma=1,field='length',name='proximity',nax_num=10):
	'''
	For each edge in a graph, add proximity.
	Proximity is defined as
	...
	for node_0,edges in graph._adj.items():
		...
		proximity=np.zeros(len(edges))

		for idx,items in edges.items():

			node_1,edge=items

			proximity[idx]=np.exp(-edge[field]/gamma)*(edge[field]<=max_value)
			edge[name]=proximity
